    ...
    <main className="min-h-[calc(100vh-4rem)] bg-gradient-to-b from-slate-950 via-slate-900 to-slate-900 text-slate-100">
      <section className="max-w-5xl mx-auto px-6 py-20 space-y-12">
        <header className="text-center space-y-4">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold leading-tight">Reference Notes</h1>
          <p className="text-slate-200/90 max-w-3xl mx-auto">
            Quick reference for course details and the sources used to build the timeline.
          </p>
        </header>

        <section className="grid gap-6 sm:grid-cols-2">
          {info.map((val) => (
            <article key={val.label} className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
          ...
          <ul className="space-y-4 text-slate-100/90">
            {sources.map((source) => (
              <li key={source.label} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <a className="text-blue-200 hover:text-blue-100 break-all" href={source.url} target="_blank" rel="noreferrer">
                  {source.url}
                </a>
              </li>
            ))}
