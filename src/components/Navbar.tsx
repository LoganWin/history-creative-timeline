        ...
        <nav className="hidden md:flex items-center gap-8 text-sm text-gray-200/90">
          {[
            ...
            <Link
              key={link.to}
              to={link.to}
              className="group relative px-1 py-0.5 transition text-gray-200/90 hover:text-white"
            >
              {link.label}
              <span className="pointer-events-none absolute inset-x-0 bottom-0 h-0.5 bg-white/80 scale-x-0 origin-center transition-transform duration-300 group-hover:scale-x-100" />
            </Link>
          ))}
        </nav>
      </div>
